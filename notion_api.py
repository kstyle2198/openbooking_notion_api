...
# print(NOTION_TOKEN, DATABASE_ID)
...
def delete_booking(고유번호: str, 사번: str, booking_date: str):
    try:  
        target = get_pages()
        df = read_as_df(target)
        print(df.loc[(df["고유번호"]==고유번호)]["사번"].values[0], df.loc[(df["고유번호"]==고유번호)]["booking_date"].values[0])
...
def read_as_df(target):
    ...
    for page in pages:
        ...
        df = pd.DataFrame(list(zip(예약번호_list, 고유번호_list, company_list, 사번_list, name_list, table_num_list, booking_date_list, status_list)), 
                        columns=['예약번호', '고유번호', 'company', '사번', 'name', 'table_number', 'booking_date', 'status'])
    return df
